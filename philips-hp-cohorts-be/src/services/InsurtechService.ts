...
static async getUserGrades(instanceId: string) {
    try {
        ...
        const userGrades = await InsurtechData.aggregate(pipeline);
    ...
    private static getCohortStressLevel(grade: number): string {
        ...
        if (roundedGrade <= 1) return 'Very High';
        if (roundedGrade === 2) return 'High';
        if (roundedGrade === 3) return 'Normal';
        if (roundedGrade === 4) return 'Low';
        if (roundedGrade >= 5) return 'Very Low';
